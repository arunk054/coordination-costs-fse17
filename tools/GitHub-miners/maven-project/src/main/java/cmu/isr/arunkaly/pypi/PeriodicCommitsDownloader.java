...
public class PeriodicCommitsDownloader {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < reposArr.length; ++i) {
			...
			if (ownerRepo.length!=2)
				continue;
			WrapperCommitsMiner wp = new WrapperCommitsMiner(ownerRepo[0], ownerRepo[1]);
			
			wp.invokeAllMiners(fileController);
			count++;
		}
		System.out.println("Total Git repos in file: "+reposArr.length);
		System.out.println("Total repos with commits downloaded: "+ count);
	}
