...
def writeToCSV(reposFullDict, repoPkgMap, builtins, dictWords, cranPkgList, outFileName):
	#Create tuples out of the data structures then it is just a matter of dumping it to the file
	#Header looks like
	header = ['repo', 'issue_id', 'is_open', 'is_pullrequest', 'is_feature', 'is_external','deps_count',
	 'is_external_wo_builtin','is_external_with_builtin', 'is_external_wo_eng_words','is_external_with_eng_words',
	  'is_external_wo_eng_words_wo_builtin','is_external_wo_eng_words_with_builtin',
	  'int_refs','ext_refs','crossrefs', 'time_in_secs', 'comments', 'users', 'is_reporter_developer', 
	  'repo_age_secs',  'repo_stars', 'repo_watchers','repo_forks', 'repo_is_fork', 'repo_size_kb',
	   'repo_contributors', 'reporter','pkgName', 'repo_is_cran', 'is_external_wo_cran', 'is_external_with_cran',
	   'is_external_wo_eng_words_wo_cran', 'is_external_wo_eng_words_with_cran']
	...
	with open(outFileName, 'w') as outCSV:
		...
		for repo,repoDict in reposFullDict.items():
			for issueId, issueDict in repoDict.items():
				...
				row.append(True if pkgName in cranPkgList else False)

				depsDict1 = filterWords(depsDict, cranPkgList, False, False)
				row.append(False if len(depsDict1) == 0 else True)
				depsDict1 = filterWords(depsDict, cranPkgList, True, False)
				row.append(False if len(depsDict1) == 0 else True)

				depsDict1 = filterWords(depsDict, dictWords, False, True)
				depsDict2 = filterWords(depsDict1, cranPkgList, False, False)
				row.append(False if len(depsDict2) == 0 else True)
				depsDict2 = filterWords(depsDict1, cranPkgList, True, False)
				row.append(False if len(depsDict2) == 0 else True)
...
def getArgs():
	...
	parser.add_argument('cran_packages',help='')	
...
def getCranPackages(fileName) :
	pkgList = []
	with open(fileName, 'r', encoding='utf-8', errors='ignore') as myCSV:
		curReader = csv.reader(myCSV)
		next(curReader)
		count = 1
		for row in curReader:
			if count > 400 and count < 405:
				print(row)
			count+=1
			pkgList.append(row[0].lower())
	return pkgList

def main():
	args = getArgs()
	cranPkgList = getCranPackages(args.cran_packages)
	reposFullDict = dict()
	...
	writeToCSV(reposFullDict, repoPkgMap, builtIns, dictWords, cranPkgList, args.outFileName)
